...
import games.bahasa.imdbmovie.model.DataBackdrop
...
class DetailTvActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (detailMovie != null) {
            ...
            imgTvShow.getResponse().observeOnce(this) {
                if (it?.isNotEmpty() == true && it.size < 5) {
                    setSlider(it,it.size)
                }else if (it?.isNotEmpty() == true && it.size > 5) {
                    setSlider(it,5)
                }
            }
        }
    ...
    fun setSlider(list: List<DataBackdrop>, max: Int) {
        list.subList(0, max).forEach { draw ->
